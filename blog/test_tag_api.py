...
class TagApiTestCase(LiveServerTestCase):
  def setUp(self):
    get_user_model().objects.create_user(email="testuser@example.com",
                                         password="password1")
  ...
  def test_tag_create_basic_auth(self):
    self.client.auth = HTTPBasicAuth("testuser@example.com", "password1")
    resp = self.client.post(self.live_server_url + "/api/v1/tags/",
                            {"value": "tag5"})
  ...
  def test_tag_create_token_auth(self):
    token_resp = self.client.post(
      ...
      {"username": "testuser@example.com", "password": "password1"}
    )
